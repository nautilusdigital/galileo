...
wallet (wallet_a)
{
	...
	version = new QLabel (boost::str (boost::format ("Version %1%.%2%") % GALILEO_VERSION_MAJOR % GALILEO_VERSION_MINOR).c_str ());
...
mgalileo (new QRadioButton ("Mxrb")),
kgalileo (new QRadioButton ("kxrb")),
...
wallet (wallet_a)
{
	ratio_group->addButton (mgalileo);
	ratio_group->addButton (kgalileo);
	ratio_group->addButton (galileo);
	ratio_group->setId (mgalileo, 0);
	ratio_group->setId (kgalileo, 1);
	ratio_group->setId (galileo, 2);
	scale_layout->addWidget (scale_label);
	scale_layout->addWidget (mgalileo);
	scale_layout->addWidget (kgalileo);
	...
	QObject::connect (mgalileo, &QRadioButton::toggled, [this]() {
		if (mgalileo->isChecked ())
		{
			this->wallet.change_rendering_ratio (galileo::Mxrb_ratio);
		}
	});
	QObject::connect (kgalileo, &QRadioButton::toggled, [this]() {
		if (kgalileo->isChecked ())
		{
